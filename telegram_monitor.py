...
class TelegramMonitor:
 ...
 max_actions=config.max_comments_per_hour,
    ...
    async def _initialize_client(self):
        """Initialize the Telegram client with session conflict handling"""
        ...
        for attempt in range(max_retries):
            try:
                ...
                self.logger.warning(f"Session conflict detected (attempt {attempt + 1}/{max_retries})")
                ...
                if attempt < max_retries - 1:
                    ...
                    if self.config.session_string:
                        session = StringSession()  # Empty session to force re-auth
                        self.logger.info("Creating fresh session for retry")
                else:
                    self.logger.error("Max retries reached for session conflict")
                    raise

            except Exception as e:
                if attempt < max_retries - 1:
                    self.logger.warning(f"Connection failed (attempt {attempt + 1}/{max_retries}): {e}")
                    await asyncio.sleep(retry_delay)
                else:
                    raise
